...
public class DCCManager extends Service {
	...
	public static void addRequest(PrivateMessageEvent event) {
		...
		if (!alreadyQueuedPackNumbers.isEmpty()) {
			dccUser.sendUserNotice(
					"You are already queued for pack" + ((alreadyQueuedPackNumbers.size() > 1) ? "s" : "") + " "
		...
		if (!packNumbers.isEmpty()) {
			dccUser.sendUserNotice("Unknown pack" + ((packNumbers.size() > 1) ? "s" : "") + " "
		...
		dccUser.sendUserNotice("You are queued for " + dccUser.getTransfers().size() + " pack(s)");
	...
	private static DCCRequest getNextForQueue() {
		...
		// Get the user who hasn't transferred recently and has less than 2 transfers
		// running
