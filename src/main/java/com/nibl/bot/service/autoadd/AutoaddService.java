...
public class AutoaddService extends Service {
    ...
    private static Path rootScanPath;
    private static Path stateFile = Paths.get("state" + File.separator + "state.json");
    ...
    public AutoaddService(Bot myBot) {
        ...
        rootScanPath = Paths.get(myBot.getProperty("data_directory"));
    }
    ...
    public static void autoAddScan(Path path) throws IOException {
        ...
        if (!stateFile.getParent().toFile().exists()) {
            stateFile.getParent().toFile().mkdir();
        }
